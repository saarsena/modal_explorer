...
`;var q=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],de={Major:"major",Minor:"natural_minor",Dorian:"dorian",Phrygian:"phrygian",Lydian:"lydian",Mixolydian:"mixolydian",Locrian:"locrian","Harmonic Minor":"harmonic_minor","Melodic Minor":"melodic_minor"},ut={C:0,"C#":1,Db:1,D:2,"D#":3,Eb:3,E:4,F:5,"F#":6,Gb:6,G:7,"G#":8,Ab:8,A:9,"A#":10,Bb:10,B:11},gt=[0,7,2,9,4,11,6,1,8,3,10,5],Z=[0,2,4,5,7,9,11],bt=[0,2,3,5,7,8,10],ft=[0,2,3,5,7,9,10],vt=[0,1,3,5,7,8,10],yt=[0,2,4,6,7,9,11],xt=[0,2,4,5,7,9,10],wt=[0,1,3,5,6,8,10],kt=[0,2,3,5,7,8,11],Ct=[0,2,3,5,7,9,11],It=[0,2,4,7,9],Mt=[0,3,5,7,10],St=[0,3,5,6,7,10],Q={major:Z,natural_minor:bt,dorian:ft,phrygian:vt,lydian:yt,mixolydian:xt,locrian:wt,harmonic_minor:kt,melodic_minor:Ct,pentatonic_major:It,pentatonic_minor:Mt,blues:St},Et={major:"major",natural_minor:"minor",dorian:"Dorian",phrygian:"Phrygian",lydian:"Lydian",mixolydian:"Mixolydian",locrian:"Locrian",harmonic_minor:"harmonic minor",melodic_minor:"melodic minor",pentatonic_major:"pentatonic major",pentatonic_minor:"pentatonic minor",blues:"blues"},Oe=[{id:"major",label:"Major"},{id:"natural_minor",label:"Natural Minor"},{id:"dorian",label:"Dorian"},{id:"phrygian",label:"Phrygian"},{id:"lydian",label:"Lydian"},{id:"mixolydian",label:"Mixolydian"},{id:"locrian",label:"Locrian"},{id:"harmonic_minor",label:"Harmonic Minor"},{id:"melodic_minor",label:"Melodic Minor"},{id:"pentatonic_major",label:"Pentatonic Major"},{id:"pentatonic_minor",label:"Pentatonic Minor"},{id:"blues",label:"Blues"}];function G(e,t=.25){let c=new Map;for(let g of e){let m=Math.round(g.startTime/t)*t,d=Math.round(m*1e3),o=c.get(d);o?o.push(g.pitch):c.set(d,[g.pitch])}return c}function ae(e){return JSON.stringify(e).replace(/</g,"\\u003C").replace(/>/g,"\\u003E")}function Tt(e,t,c){let g=t/100,m=c/100,d=g*Math.min(m,1-m),o=n=>{let i=(n+e/30)%12;return m-d*Math.max(Math.min(i-3,9-i,1),-1)};return Math.round(o(0)*255)<<16|Math.round(o(8)*255)<<8|Math.round(o(4)*255)}function Y(e){let t=e.match(/^([A-G][#b]?)\s+(.*)/);if(!t)return null;let c=ut[t[1]??""];if(c===void 0)return null;let g=/minor|min\b/i.test(t[2]??""),m=(gt[c]??0)*30;return Tt(m,g?55:70,g?35:45)}async function U(e,t){let c=G(e),m=(await Promise.all([...c.entries()].map(([i,s])=>t.send("recognize_chord",{notes:s}).then(u=>({beatKey:i,r:u}))))).sort((i,s)=>i.beatKey-s.beatKey).flatMap(({r:i})=>{let s=i.matches[0];return s&&s.score>=.5?[s.chord.name]:[]}).filter((i,s,u)=>s===0||i!==u[s-1]);if(m.length>0){let s=(await t.send("analyze",{chord_names:m})).inferred_key;return{root:s.root,scale:s.scale,label:s.label}}let d=new Array(12).fill(0);for(let i of e){let s=i.pitch%12;d[s]=(d[s]??0)+1}let o=-1,n=null;for(let i=0;i<12;i++)for(let[s,u]of Object.entries(Q)){let a=0;for(let r of u)a+=d[(i+r)%12]??0;if(a>o){o=a;let r=q[i]??"C",l=Et[s]??s;n={root:i,scale:s,label:`${r} ${l}`}}}return n}function me(e,t){let c=e%12;if(t.has(c))return e;for(let g=1;g<=6;g++){let m=t.has((c+g)%12),d=t.has((c-g+12)%12);if(m&&d||m)return e+g;if(d)return e-g}return e}function X(e){let t=e.match(/^([A-G][#b]?)\s+(.+)$/);if(!t)return e;let c=t[1]??e,g=(t[2]??"").toLowerCase().trim();if(g==="major")return c;if(g==="minor"||g==="natural minor"||g==="natural_minor")return`${c}m`;if(g==="harmonic minor"||g==="harmonic_minor")return`${c}hm`;if(g==="melodic minor"||g==="melodic_minor")return`${c}mm`;let m=g.split(/[\s_]/)[0]??g;return`${c} ${m.slice(0,3)}`}function _t(e,t,c,g){return e===c&&t===g?"Same key":e===c?t?"Parallel major":"Parallel minor":!t&&g&&c===(e+9)%12?"Relative minor":t&&!g&&c===(e+3)%12?"Relative major":c===(e+7)%12?"Dominant (V)":c===(e+5)%12?"Subdominant (IV)":null}var Nt=/^[b#♭♯]?[ivIV]/;async function He(e,t,c,g){let m=async()=>(await e.send("parse_roman",{roman:t,key:c,scale:g})).chord,d=async()=>(await e.send("parse_chord",{name:t})).chord,[o,n]=Nt.test(t)?[m,d]:[d,m];try{return await o()}catch{try{return await n()}catch{throw new Error(`Couldn't parse "${t}" as a Roman numeral or chord name`)}}}async function $e(e,t){let c=t.customProgression?.trim();if(!c){let{chords:m}=await e.send("progression",{key:t.key,scale:t.scale,template:t.template,sevenths:t.sevenths});return m}let g=c.split(/[\s,|]+/).filter(Boolean);return Promise.all(g.map(m=>He(e,m,t.key,t.scale)))}function je(e){return e.customProgression?.trim()||e.template}var At={halves:{span:4,hits:[{t:0,d:2,v:1},{t:2,d:2,v:.9}]},quarters:{span:4,hits:[{t:0,d:1,v:1},{t:1,d:1,v:.85},{t:2,d:1,v:.95},{t:3,d:1,v:.85}]},pump_8ths:{span:4,hits:[0,.5,1,1.5,2,2.5,3,3.5].map(e=>({t:e,d:.45,v:e%1===0?1:.8}))},swung_8ths:{span:4,hits:[0,1,2,3].flatMap(e=>[{t:e,d:.6,v:1},{t:e+.66,d:.3,v:.75}])},charleston:{span:4,hits:[{t:0,d:1.4,v:1},{t:1.5,d:2.4,v:.85}]},tresillo:{span:4,hits:[{t:0,d:1.4,v:1},{t:1.5,d:1.4,v:.85},{t:3,d:.9,v:.95}]},boom_chuck:{span:4,hits:[{t:0,d:.95,v:1},{t:1,d:.45,v:.75},{t:1.5,d:.45,v:.75},{t:2,d:.95,v:.95},{t:3,d:.45,v:.75},{t:3.5,d:.45,v:.75}]},syncopated:{span:4,hits:[{t:0,d:.7,v:1},{t:.75,d:.2,v:.7},{t:1,d:.45,v:.85},{t:1.5,d:1.4,v:.95},{t:3,d:.45,v:.85},{t:3.5,d:.45,v:.8}]},offbeats:{span:4,hits:[.5,1.5,2.5,3.5].map(e=>({t:e,d:.35,v:.95}))},arp_up_8ths:{span:4,arp:!0,hits:[0,.5,1,1.5,2,2.5,3,3.5].map(e=>({t:e,d:.5,v:e%1===0?.95:.8}))}},he=90;function se(e,t,c){let g=c?At[c]:void 0;if(!g)return e.flatMap((d,o)=>d.map(n=>({pitch:Math.max(0,Math.min(127,n)),startTime:o*t,duration:t*.95,velocity:he})));let m=t/g.span;return e.flatMap((d,o)=>{if(d.length===0)return[];let n=[...d].sort((i,s)=>i-s);return g.hits.flatMap((i,s)=>{let u=Math.max(1,Math.min(127,Math.round(he*i.v)));return(g.arp?[n[s%n.length]??60]:n).map(r=>({pitch:Math.max(0,Math.min(127,r)),startTime:o*t+i.t*m,duration:i.d*m,velocity:u}))})})}function Rt(e,t){return e.map((c,g)=>{let m=e[g+1],d=m?m.beat:Math.max(t,c.beat+1),o=c.pitchClasses,n=(i,s)=>{let u=o[i];return u===void 0?s:(u-c.root+12)%12||s};return{start:c.beat,span:Math.max(.5,d-c.beat),rootPc:c.root,thirdIv:n(1,4),fifthIv:n(2,7),nextRootPc:m?m.root:null}})}function pe(e,t){return Math.abs(e-1-t)<=Math.abs(e+1-t)?e-1:e+1}function Le(e,t,c){let g=12*(c+1),m=o=>Math.max(20,Math.min(64,o)),d=(o,n,i,s)=>({pitch:m(i),startTime:o,duration:n,velocity:Math.max(1,Math.min(127,Math.round(he*s)))});return e.flatMap(o=>{let n=g+o.rootPc,i=n+o.thirdIv,s=n+o.fifthIv,u=[];switch(t){case"root_fifth":{if(o.span<2){u.push(d(o.start,o.span*.95,n,1));break}let a=o.span/2;u.push(d(o.start,a*.95,n,1)),u.push(d(o.start+a,a*.95,s,.85));break}case"pump_8ths":case"root_octave_8ths":{let a=Math.max(1,Math.round(o.span/.5)),r=o.span/a;for(let l=0;l<a;l++){let p=t==="root_octave_8ths"&&l%2===1?n+12:n;u.push(d(o.start+l*r,r*.9,p,l%2===0?1:.8))}break}case"walking":{let a=Math.max(1,Math.round(o.span)),r=o.span/a,l=[n,i,s,i];for(let p=0;p<a;p++){let f=l[p%l.length]??n;p===a-1&&a>1&&o.nextRootPc!==null&&(f=pe(g+o.nextRootPc,s)),u.push(d(o.start+p*r,r*.9,f,p===0?1:.88))}break}case"two_feel":{if(o.span<2){u.push(d(o.start,o.span*.95,n,1));break}let a=Math.max(1,Math.round(o.span/2)),r=o.span/a;for(let l=0;l<a;l++){let p=l%2===0?n:s;l===a-1&&a>1&&o.nextRootPc!==null&&(p=pe(g+o.nextRootPc,s)),u.push(d(o.start+l*r,r*.92,p,l%2===0?1:.85))}break}case"boogie":{let a=[n,i,s,n+9,n+10,n+9,s,i],r=Math.max(1,Math.round(o.span/.5)),l=o.span/r;for(let p=0;p<r;p++)u.push(d(o.start+p*l,l*.9,a[p%a.length]??n,p%2===0?1:.8));break}case"bossa":{let a=o.span/4;u.push(d(o.start+0*a,1.4*a,n,1)),u.push(d(o.start+1.5*a,.4*a,s,.8)),u.push(d(o.start+2*a,1.4*a,n,.95)),u.push(d(o.start+3.5*a,.4*a,s,.8));break}case"tumbao":{let a=o.span/4,r=o.nextRootPc!==null?g+o.nextRootPc:n;u.push(d(o.start+0*a,.4*a,n,.7)),u.push(d(o.start+1.5*a,1.3*a,s,.95)),u.push(d(o.start+3*a,.95*a,r,1));break}case"quarter_pulse":{let a=Math.max(1,Math.round(o.span)),r=o.span/a;for(let l=0;l<a;l++)u.push(d(o.start+l*r,r*.9,n,l===0?1:.88));break}case"reggaeton":{let a=Math.max(1,Math.round(o.span/2)),r=o.span/a,l=r/2;for(let p=0;p<a;p++){let f=o.start+p*r;u.push(d(f+0*l,.7*l,n,p===0?1:.95)),u.push(d(f+.75*l,.7*l,n,.8)),u.push(d(f+1.5*l,.45*l,n,.85))}break}case"funk_16ths":{let a=o.span/4,r=[[0,.4,n,1],[.75,.2,n,.6],[1,.4,n+12,.9],[1.75,.2,n,.6],[2,.4,n,1],[2.75,.2,n,.6],[3,.4,n+12,.9],[3.5,.4,n,.8]];for(let[l,p,f,x]of r)u.push(d(o.start+l*a,p*a,f,x));break}case"pedal_pickup":{if(o.nextRootPc===null||o.span<=1){u.push(d(o.start,o.span*.95,n,1));break}u.push(d(o.start,o.span-.5,n,1)),u.push(d(o.start+o.span-.5,.4,pe(g+o.nextRootPc,n),.85));break}case"tresillo":{let a=o.span/4;u.push(d(o.start+0*a,1.4*a,n,1)),u.push(d(o.start+1.5*a,1.4*a,n,.85)),u.push(d(o.start+3*a,.9*a,s,.95));break}case"offbeats":{let a=o.span/4;for(let r of[.5,1.5,2.5,3.5])u.push(d(o.start+r*a,.35*a,n,.95));break}default:u.push(d(o.start,o.span*.95,n,1))}return u})}function Pt(e){let t=Me(e,"1.0.0"),c=process.env.COMPOSITION_AIDE_PATH??Ve.join(__dirname,"..","engine"),g=process.env.PYTHON_CMD??(process.platform==="win32"?"python":"python3"),m=new ne(c,g);m.send("list_ops").catch(()=>{}),t.commands.registerCommand("aide.generate",d=>{(async o=>{let n=o.selected_lanes.map(x=>t.getObjectFromHandle(x,$)).filter(x=>x instanceof F);if(!n.length){console.log("[composition-aide] No MIDI tracks in selection.");return}let i=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(ce)}`,380,416),s;try{s=JSON.parse(i)}catch{return}if(!s)return;let u=o.time_selection_end-o.time_selection_start,a=await $e(m,s),{voicings:r}=await m.send("voice_progression",{chords:a,strategy:s.voicing,octave:4}),l=u/a.length,p=`${je(s)} \u2014 ${s.keyName} ${s.scale.replace(/_/g," ")}`,f=se(r,l,s.rhythm);if(s.snapToScale){let x=Q[s.scale]??Z,k=new Set(x.map(y=>(s.key+y)%12));f=f.map(y=>({...y,pitch:me(y.pitch,k)}))}await Promise.all(n.map(async x=>{let k=await x.createMidiClip(o.time_selection_start,u);k.name=p,k.notes=f})),console.log(`[composition-aide] "${p}": ${a.map(x=>x.name).join(" \u2013 ")} (${l.toFixed(2)} beats/chord)`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiTrack.ArrangementSelection","Generate Progression\u2026","aide.generate"),t.commands.registerCommand("aide.analyze",d=>{let o=async n=>{let i=t.getObjectFromHandle(n,B),s=i.notes;if(s.length===0){console.log("[composition-aide] Clip has no notes.");return}let u=G(s),r=(await Promise.all([...u.entries()].map(([S,R])=>m.send("recognize_chord",{notes:R}).then(j=>({beatKey:S,result:j}))))).sort((S,R)=>S.beatKey-R.beatKey).flatMap(({beatKey:S,result:R})=>{let j=R.matches[0];return!j||j.score<.5?[]:[{beat:S/1e3,name:j.chord.name,score:j.score,pitchClasses:j.chord.pitch_classes,root:j.chord.root,quality:j.chord.quality}]}).filter((S,R,j)=>R===0||S.name!==j[R-1]?.name);if(r.length===0){console.log("[composition-aide] Could not identify any chords in this clip.");return}let l=r.map(S=>S.name),p=await m.send("analyze",{chord_names:l}),f={clipName:i.name||"(unnamed clip)",noteCount:s.length,inferredKey:p.inferred_key.label,scaleRoot:p.inferred_key.root,scaleIntervals:[...Q[p.inferred_key.scale]??Z],chords:l.map((S,R)=>({beat:r[R]?.beat??0,name:S,roman:p.roman_labels[R]??S,tension:p.tension[R]??0,score:r[R]?.score??0,pitchClasses:r[R]?.pitchClasses??[],root:r[R]?.root??0,quality:r[R]?.quality??"major"})),substitutions:p.substitutions.slice(0,5).map(S=>({position:S.position,original:S.original.name,replacement:S.replacement.name,rationale:S.rationale})),summary:p.summary},x=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Ee.replace("__ANALYSIS_JSON__",ae(f)))}`,520,560);console.log(`[composition-aide] Analyzed "${f.clipName}": ${f.inferredKey} \u2014 ${l.join(" \u2013 ")}`);let k;try{k=JSON.parse(x)}catch{return}if(!k||k.action!=="substitute")return;let{position:y,original:w,replacement:E}=k,_=r[y];if(!_){console.error(`[composition-aide] Substitution position ${y} out of range.`);return}let h=r[y+1],b=_.beat,C=i.notes,v=Math.max(b+4,...C.map(S=>S.startTime+S.duration)),I=h?.beat??v,M=C.filter(S=>S.startTime>=b-.01&&S.startTime<I-.01),T=M.length>0?Math.round(M.reduce((S,R)=>S+R.pitch,0)/M.length):60,P=Math.max(3,Math.min(6,Math.floor(T/12)-1)),O=await m.send("voicings",{name:E,octave:P}),A=M.length>0?M.reduce((S,R)=>S+R.duration,0)/M.length:(I-b)*.95,N=O.close.map(S=>({pitch:Math.max(0,Math.min(127,S)),startTime:b,duration:A,velocity:90}));i.notes=[...C.filter(S=>S.startTime<b-.01||S.startTime>=I-.01),...N],console.log(`[composition-aide] Substituted: ${w} \u2192 ${E} at beat ${b.toFixed(2)} (octave ${P})`),await o(n)};o(d).catch(n=>console.error(n))}),t.ui.registerContextMenuAction("MidiClip","Analyze Harmony\u2026","aide.analyze"),t.commands.registerCommand("aide.chordPalette",d=>{(async o=>{let n=t.getObjectFromHandle(o,J),i=t.application.song,s=i?.rootNote??0,u=i?.scaleName??"Major",a=de[u]??"major",r=await Promise.all(Oe.flatMap(({id:I})=>[m.send("diatonic",{key:s,scale:I,sevenths:!1}).then(M=>({id:I,sevenths:!1,chords:M.chords})),m.send("diatonic",{key:s,scale:I,sevenths:!0}).then(M=>({id:I,sevenths:!0,chords:M.chords}))])),l={};for(let{id:I,sevenths:M,chords:T}of r)l[I]||(l[I]={triads:[],sevenths:[],intervals:Q[I]??Z}),M?l[I].sevenths=T:l[I].triads=T;let p={keyRoot:s,keyName:q[s]??"C",defaultScale:a,scaleOptions:Oe,scales:l},f=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Te.replace("__PALETTE_JSON__",ae(p)))}`,510,296),x;try{x=JSON.parse(f)}catch{return}if(!x||x.action!=="insert")return;let{chordName:k,length:y,voicing:w,octave:E,selectedScale:_}=x,C=(await m.send("voicings",{name:k,octave:E}))[w].map(I=>({pitch:Math.max(0,Math.min(127,I)),startTime:0,duration:y*.95,velocity:90})),v=n.clip;if(v instanceof B)v.name=k,v.notes=C;else{let I=await n.createMidiClip(y);I.name=k,I.notes=C}console.log(`[composition-aide] Inserted ${k} (${w}, oct ${E}, ${y} beats) \u2014 ${p.keyName} ${_}`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("ClipSlot","Insert Chord\u2026","aide.chordPalette"),t.commands.registerCommand("aide.sessionMap",d=>{(async o=>{let n=t.application.song;if(!n)return;let i=n.tracks,u=n.scenes.map(h=>h.name),a=[];for(let h=0;h<i.length;h++){let b=i[h];if(!b)continue;let C=b.clipSlots;for(let v=0;v<C.length;v++){let I=C[v];if(!I)continue;let M=I.clip;if(!(M instanceof B))continue;let T=M.notes;T.length!==0&&a.push({trackIndex:h,trackName:b.name,sceneIndex:v,notes:T,clipName:M.name||"(unnamed)"})}}let r=a.length;if(r===0){console.log("[composition-aide] No MIDI clips with notes found in session.");return}let p=a.reduce((h,b)=>Math.max(h,b.sceneIndex),0)+1,f=[];await t.ui.withinProgressDialog(`Analyzing session \u2014 ${r} clips\u2026`,{progress:0},async(h,b)=>{let C=0;await Promise.all(a.map(async v=>{if(b.aborted)return;let I=null,M=0;try{let T=G(v.notes),O=(await Promise.all([...T.entries()].map(([A,N])=>m.send("recognize_chord",{notes:N}).then(S=>({beatKey:A,r:S}))))).sort((A,N)=>A.beatKey-N.beatKey).flatMap(({beatKey:A,r:N})=>{let S=N.matches[0];return!S||S.score<.5?[]:[{beat:A/1e3,name:S.chord.name}]}).filter((A,N,S)=>N===0||A.name!==S[N-1]?.name);if(O.length>0){let A=await m.send("analyze",{chord_names:O.map(N=>N.name)});I=A.inferred_key.label,M=A.inferred_key.score}}catch(T){console.error(`[composition-aide] Could not analyze ${v.clipName}:`,T)}f.push({...v,key:I,score:M}),C++,await h(`${C} / ${r} clips\u2026`,Math.round(C/r*100))}))});let x=new Map;for(let h of f)h.key&&x.set(h.key,(x.get(h.key)??0)+1);let k=null,y=0;for(let[h,b]of x)b>y&&(y=b,k=h);let E=[...new Set(f.map(h=>h.trackIndex))].sort((h,b)=>h-b).map(h=>{let b=f.find(v=>v.trackIndex===h)?.trackName??`Track ${h+1}`,C=Array(p).fill(null);for(let v of f.filter(I=>I.trackIndex===h))C[v.sceneIndex]={clipName:v.clipName,key:v.key,score:v.score};return{name:b,clips:C}}),_={dominantKey:k,sceneCount:p,sceneNames:u.slice(0,p),totalMidiClips:r,analyzedClips:f.filter(h=>h.key!==null).length,tracks:E};await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(_e.replace("__SESSION_JSON__",ae(_)))}`,680,480),console.log(`[composition-aide] Session map: ${_.analyzedClips}/${r} clips`+(k?` \u2014 dominant key: ${k}`:""))})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("Scene","Map Session Keys\u2026","aide.sessionMap"),t.commands.registerCommand("aide.colorByKey",d=>{(async o=>{let n=t.application.song;if(!n)return;let i=[];for(let u of n.tracks)for(let a of u.clipSlots){let r=a.clip;if(!(r instanceof B))continue;let l=r.notes;l.length!==0&&i.push({clip:r,notes:l,clipName:r.name||"(unnamed)"})}if(i.length===0){console.log("[composition-aide] No MIDI clips with notes found.");return}let s=0;await t.ui.withinProgressDialog(`Coloring ${i.length} clips by key\u2026`,{progress:0},async(u,a)=>{let r=0;await Promise.all(i.map(async l=>{if(!a.aborted){try{let p=G(l.notes),x=(await Promise.all([...p.entries()].map(([k,y])=>m.send("recognize_chord",{notes:y}).then(w=>({beatKey:k,r:w}))))).sort((k,y)=>k.beatKey-y.beatKey).flatMap(({r:k})=>{let y=k.matches[0];return y&&y.score>=.5?[y.chord.name]:[]}).filter((k,y,w)=>y===0||k!==w[y-1]);if(x.length>0){let k=await m.send("analyze",{chord_names:x}),y=Y(k.inferred_key.label);y!==null&&(l.clip.color=y,s++)}}catch(p){console.error(`[composition-aide] Could not color "${l.clipName}":`,p)}r++,await u(`${r} / ${i.length} clips\u2026`,Math.round(r/i.length*100))}}))}),console.log(`[composition-aide] Colored ${s} / ${i.length} clips by key.`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("Scene","Color Clips by Key","aide.colorByKey"),t.commands.registerCommand("aide.generateInClip",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.looping?n.loopEnd-n.loopStart:n.duration,s=i>0?i:8,u=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(ce)}`,380,416),a;try{a=JSON.parse(u)}catch{return}if(!a)return;let r=await $e(m,a),{voicings:l}=await m.send("voice_progression",{chords:r,strategy:a.voicing,octave:4}),p=s/r.length,f=se(l,p,a.rhythm);if(a.snapToScale){let x=Q[a.scale]??Z,k=new Set(x.map(y=>(a.key+y)%12));f=f.map(y=>({...y,pitch:me(y.pitch,k)}))}n.notes=f,n.name=`${je(a)} \u2014 ${a.keyName} ${a.scale.replace(/_/g," ")}`,console.log(`[composition-aide] Generated "${n.name}": ${r.map(x=>x.name).join(" \u2013 ")} (${p.toFixed(2)} beats/chord)`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Fill Clip with Progression\u2026","aide.generateInClip"),t.commands.registerCommand("aide.voiceLead",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=G(i),a=(await Promise.all([...s.entries()].map(([w,E])=>m.send("recognize_chord",{notes:E}).then(_=>({beatKey:w,r:_}))))).sort((w,E)=>w.beatKey-E.beatKey).flatMap(({beatKey:w,r:E})=>{let _=E.matches[0];return!_||_.score<.5?[]:[{beat:w/1e3,name:_.chord.name}]}).filter((w,E,_)=>E===0||w.name!==_[E-1]?.name);if(a.length===0){console.log("[composition-aide] No chords identified in clip.");return}let r=Math.round(i.reduce((w,E)=>w+E.pitch,0)/i.length),l=Math.max(3,Math.min(6,Math.floor(r/12)-1)),{voicings:p}=await m.send("voice_progression",{chords:a.map(w=>w.name),strategy:"smooth",octave:l}),f=new Map;for(let w of i){let E=Math.round(w.startTime/.25)*.25,_=Math.round(E*1e3),h=f.get(_)??[];h.push(w),f.set(_,h)}let x=new Set(a.map(w=>Math.round(w.beat*1e3))),k=[];for(let w=0;w<a.length;w++){let E=a[w],_=p[w]??[];if(!E||_.length===0)continue;let h=Math.round(E.beat*1e3),b=f.get(h)??[],C=b.length>0?b.reduce((I,M)=>I+M.duration,0)/b.length:2,v=b.length>0?Math.round(b.reduce((I,M)=>I+(M.velocity??90),0)/b.length):90;for(let I of _)k.push({pitch:Math.max(0,Math.min(127,I)),startTime:E.beat,duration:C,velocity:v})}let y=i.filter(w=>{let E=Math.round(Math.round(w.startTime/.25)*.25*1e3);return!x.has(E)});n.notes=[...y,...k],console.log(`[composition-aide] Voice-led "${n.name||"(unnamed)"}": ${a.length} chords, octave ${l}`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Optimize Voice Leading","aide.voiceLead"),t.commands.registerCommand("aide.snapToKey",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=await U(i,m);if(!s){console.log("[composition-aide] Could not determine key for this clip.");return}let u=await m.send("scale_info",{key:s.root,scale:s.scale}),a=new Set(u.notes),r=i.filter(p=>a.has(p.pitch%12)),l=i.length-r.length;n.notes=r,console.log(`[composition-aide] Snapped "${n.name||"(unnamed)"}" to ${s.label}: removed ${l} out-of-key notes, kept ${r.length}`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Snap to Key","aide.snapToKey"),t.commands.registerCommand("aide.batchTranspose",d=>{(async o=>{let n=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Ne)}`,300,200),i;try{i=JSON.parse(n)}catch{return}if(!i||i.action!=="transpose")return;let{semitones:s}=i;if(s===0)return;let u=0;for(let a of o.selected_clip_slots){let l=t.getObjectFromHandle(a,J).clip;if(!(l instanceof B))continue;let p=l.notes;p.length!==0&&(l.notes=p.map(f=>({pitch:Math.max(0,Math.min(127,f.pitch+s)),startTime:f.startTime,duration:f.duration,velocity:f.velocity??90})),u++)}console.log(`[composition-aide] Transposed ${u} clips ${s>0?"+":""}${s} semitones`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("ClipSlotSelection","Transpose Selected Clips\u2026","aide.batchTranspose"),t.commands.registerCommand("aide.findCompatible",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=await U(i,m);if(!s){console.log("[composition-aide] Could not determine key for reference clip.");return}let u=t.application.song;if(!u)return;let a=/minor|min\b/i.test(s.scale),r=[];for(let v of u.tracks)for(let I of v.clipSlots){let M=I.clip;if(!(M instanceof B))continue;let T=M.notes;T.length!==0&&r.push({trackName:v.name,clip:M,notes:T})}let l=[];await t.ui.withinProgressDialog("Scanning session for compatible clips\u2026",{progress:0},async(v,I)=>{let M=0;await Promise.all(r.map(async T=>{if(!I.aborted){try{let P=await U(T.notes,m);if(P){let O=/minor|min\b/i.test(P.scale),A=_t(s.root,a,P.root,O);A&&l.push({trackName:T.trackName,clipName:T.clip.name||"(unnamed)",key:P.label,compatibility:A,color:Y(P.label)??4473924})}}catch(P){console.error(`[composition-aide] Error scanning "${T.clip.name}":`,P)}M++,await v(`${M} / ${r.length} clips\u2026`,Math.round(M/r.length*100))}}))});let p=["Same key","Relative minor","Relative major","Parallel major","Parallel minor","Dominant (V)","Subdominant (IV)"];l.sort((v,I)=>{let M=p.indexOf(v.compatibility),T=p.indexOf(I.compatibility);return(M===-1?99:M)-(T===-1?99:T)});let f=[{root:s.root,scale:s.scale,label:s.label,relationship:"Same key"}],x=a?(s.root+3)%12:(s.root+9)%12;f.push({root:x,scale:a?"major":"natural_minor",label:`${q[x]??"C"} ${a?"major":"minor"}`,relationship:a?"Relative major":"Relative minor"});let k=(s.root+7)%12;f.push({root:k,scale:s.scale,label:`${q[k]??"C"} ${a?"minor":"major"}`,relationship:"Dominant (V)"});let y=(s.root+5)%12;f.push({root:y,scale:s.scale,label:`${q[y]??"C"} ${a?"minor":"major"}`,relationship:"Subdominant (IV)"});let w=["I-V-vi-IV","I-IV-V-I","ii-V-I"],E=[];await Promise.all(f.flatMap((v,I)=>(I===0?w:w.slice(0,2)).map(async T=>{try{let P=await m.send("progression",{key:v.root,scale:v.scale,template:T,sevenths:!1});E.push({relationship:v.relationship,keyLabel:v.label,template:T,chords:P.chords.map(O=>O.name),color:Y(v.label)??4473924})}catch{}})));let _=["Same key","Relative minor","Relative major","Dominant (V)","Subdominant (IV)"];E.sort((v,I)=>{let M=_.indexOf(v.relationship),T=_.indexOf(I.relationship);return(M===-1?99:M)-(T===-1?99:T)});let h={refClipName:n.name||"(unnamed)",refKey:s.label,results:l,suggestions:E},b=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Ae.replace("__COMPATIBLE_JSON__",ae(h)))}`,620,560),C=null;try{let v=JSON.parse(b);v&&typeof v.action=="string"&&(C=v)}catch{}if(C?.action==="writeProgression"){let v=C.suggestion,I=null;e:for(let M of u.tracks)if(M instanceof F){for(let T of M.clipSlots)if(T.clip===null){I=T;break e}}if(!I)console.log("[composition-aide] No empty MIDI slot found \u2014 add an empty slot to a MIDI track.");else{let T=v.chords.length*2,P=[];for(let A=0;A<v.chords.length;A++)try{let N=await m.send("voicings",{name:v.chords[A],octave:4});for(let S of N.close??[])P.push({pitch:Math.max(0,Math.min(127,S)),startTime:A*2,duration:2*.95,velocity:90})}catch{console.warn(`[composition-aide] Could not voice chord "${v.chords[A]}"`)}let O=await I.createMidiClip(T);O.notes=P,O.name=`${v.keyLabel} \xB7 ${v.template}`,O.color=v.color,console.log(`[composition-aide] Wrote "${O.name}" (${v.chords.join(" \u2013 ")}) to empty slot`)}}console.log(`[composition-aide] Found ${l.length} compatible clips for "${h.refClipName}" (${s.label})`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Find Compatible Clips\u2026","aide.findCompatible"),t.commands.registerCommand("aide.snapToScale",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=await U(i,m);if(!s){console.log("[composition-aide] Could not determine key for this clip.");return}let u=await m.send("scale_info",{key:s.root,scale:s.scale}),a=new Set(u.notes),r=0,l=i.map(p=>{let f=me(p.pitch,a);return f!==p.pitch&&r++,{...p,pitch:f}});n.notes=l,console.log(`[composition-aide] Snapped "${n.name||"(unnamed)"}" to ${s.label}: ${r} notes adjusted, ${i.length-r} already in scale`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Snap Notes to Scale","aide.snapToScale"),t.commands.registerCommand("aide.labelClipKey",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=await U(i,m);if(!s){console.log("[composition-aide] Could not determine key for this clip.");return}let u=(n.name||"").replace(/ \[[^\]]*\]$/,"").trim(),a=u.length>0?`${u} [${X(s.label)}]`:`[${X(s.label)}]`;n.name=a,console.log(`[composition-aide] Labeled clip: "${a}" (${s.label})`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Label with Key","aide.labelClipKey"),t.commands.registerCommand("aide.labelAllKeys",d=>{(async o=>{let n=t.application.song;if(!n)return;let i=[];for(let u of n.tracks)for(let a of u.clipSlots){let r=a.clip;if(!(r instanceof B))continue;let l=r.notes;l.length!==0&&i.push({clip:r,notes:l})}if(i.length===0){console.log("[composition-aide] No MIDI clips with notes found.");return}let s=0;await t.ui.withinProgressDialog(`Labeling ${i.length} clips\u2026`,{progress:0},async(u,a)=>{let r=0;await Promise.all(i.map(async l=>{if(!a.aborted){try{let p=await U(l.notes,m);if(p){let f=(l.clip.name||"").replace(/ \[[^\]]*\]$/,"").trim();l.clip.name=f.length>0?`${f} [${X(p.label)}]`:`[${X(p.label)}]`,s++}}catch(p){console.error(`[composition-aide] Could not label "${l.clip.name}":`,p)}r++,await u(`${r} / ${i.length} clips\u2026`,Math.round(r/i.length*100))}}))}),console.log(`[composition-aide] Labeled ${s} / ${i.length} clips with key.`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("Scene","Label All Clips with Key","aide.labelAllKeys"),t.commands.registerCommand("aide.transposeSession",d=>{(async o=>{let n=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Re)}`,300,250),i;try{i=JSON.parse(n)}catch{return}if(!i||i.action!=="transpose")return;let{semitones:s,recolor:u}=i,a=t.application.song;if(!a)return;let r=[];for(let p of a.tracks)for(let f of p.clipSlots){let x=f.clip;x instanceof B&&r.push({clip:x,clipName:x.name||"(unnamed)"})}if(r.length===0){console.log("[composition-aide] No MIDI clips found in session.");return}let l=u?`Transposing + recoloring ${r.length} clips\u2026`:`Transposing ${r.length} clips\u2026`;await t.ui.withinProgressDialog(l,{progress:0},async(p,f)=>{if(s!==0){let x=0;for(let k of r){if(f.aborted)return;let y=k.clip.notes;y.length>0&&(k.clip.notes=y.map(w=>({...w,pitch:Math.max(0,Math.min(127,w.pitch+s))}))),x++,await p(`Transposing: ${x} / ${r.length}\u2026`,Math.round(u?x/r.length*50:x/r.length*100))}}if(u&&!f.aborted){let x=0,k=0;await Promise.all(r.map(async y=>{if(!f.aborted){try{let w=y.clip.notes;if(w.length===0)return;let E=G(w),h=(await Promise.all([...E.entries()].map(([b,C])=>m.send("recognize_chord",{notes:C}).then(v=>({beatKey:b,r:v}))))).sort((b,C)=>b.beatKey-C.beatKey).flatMap(({r:b})=>{let C=b.matches[0];return C&&C.score>=.5?[C.chord.name]:[]}).filter((b,C,v)=>C===0||b!==v[C-1]);if(h.length>0){let b=await m.send("analyze",{chord_names:h}),C=Y(b.inferred_key.label);C!==null&&(y.clip.color=C,x++)}}catch{}k++,await p(`Recoloring: ${k} / ${r.length}\u2026`,50+Math.round(k/r.length*50))}})),console.log(`[composition-aide] Session transposed ${s>0?"+":""}${s} semitones, ${x} clips recolored.`)}else console.log(`[composition-aide] Session transposed ${s>0?"+":""}${s} semitones.`)})})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("Scene","Transpose Session\u2026","aide.transposeSession"),t.commands.registerCommand("aide.bassLine",d=>{(async o=>{let n=t.getObjectFromHandle(o,B),i=n.notes;if(i.length===0){console.log("[composition-aide] Clip has no notes.");return}let s=G(i),a=(await Promise.all([...s.entries()].map(([h,b])=>m.send("recognize_chord",{notes:b}).then(C=>({beatKey:h,result:C}))))).sort((h,b)=>h.beatKey-b.beatKey).flatMap(({beatKey:h,result:b})=>{let C=b.matches[0];return!C||C.score<.5?[]:[{beat:h/1e3,name:C.chord.name,root:C.chord.root,pitchClasses:C.chord.pitch_classes}]}).filter((h,b,C)=>b===0||h.name!==C[b-1]?.name);if(a.length===0){console.log("[composition-aide] Could not identify any chords in this clip.");return}let r=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(Be)}`,340,300),l;try{l=JSON.parse(r)}catch{return}if(!l||l.action!=="bass")return;let p=n.looping?n.loopEnd-n.loopStart:n.duration,f=a[a.length-1]?.beat??0,x=p>f?p:f+4,k=Rt(a,x),y=Le(k,l.pattern,l.octave),w=t.application.song,E=null;if(w){e:for(let h of w.tracks)if(h instanceof F){for(let b of h.clipSlots)if(b.clip===null){E=b;break e}}}if(!E){console.log("[composition-aide] No empty MIDI slot found \u2014 add an empty slot to a MIDI track.");return}let _=await E.createMidiClip(x);_.notes=y,_.name=`Bass (${l.pattern.replace(/_/g," ")}) \u2014 ${n.name||a.map(h=>h.name).join(" \u2013 ")}`,console.log(`[composition-aide] Bass line: ${a.map(h=>h.name).join(" \u2013 ")} (${l.pattern}, octave ${l.octave}, ${y.length} notes)`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("MidiClip","Generate Bass Line\u2026","aide.bassLine"),t.commands.registerCommand("aide.songForm",d=>{(async o=>{let n=t.getObjectFromHandle(o,W),i=t.application.song;if(!i)return;let s=i.rootNote??0,u=de[i.scaleName??"Major"]??"major",a=De.replace("</head>",`<script>window._INIT={key:${s},scale:${JSON.stringify(u)}};</script></head>`),r=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(a)}`,920,640),l;try{l=JSON.parse(r)}catch{return}if(!l||l.action!=="songform"||l.sections.length===0)return;let p=await Promise.all(l.sections.map(async h=>{let b=h.bars*4,C=Math.max(1,Math.floor(b/h.beatsPerChord)),v,I=h.customProgression.trim();if(I){let N=I.split(/[\s,|]+/).filter(Boolean);v=await Promise.all(N.map(S=>He(m,S,h.key,h.scale)))}else{let{chords:N}=await m.send("progression",{key:h.key,scale:h.scale,template:h.template,sevenths:l.sevenths});v=N}let M=[];for(let N=0;N<C;N++)M.push(v[N%v.length]);let{voicings:T}=await m.send("voice_progression",{chords:M,strategy:l.voicing,octave:4}),P=se(T,h.beatsPerChord,h.rhythm),O=null;if(h.bass!=="none"){let N=M.map((S,R)=>({start:R*h.beatsPerChord,span:h.beatsPerChord,rootPc:S.root,thirdIv:S.intervals[1]??4,fifthIv:S.intervals[2]??7,nextRootPc:M[R+1]?.root??null}));O=Le(N,h.bass,l.bassOctave)}let A=q[h.key]??"C";return{section:h,totalBeats:b,chordNotes:P,bassNotes:O,chordNames:[...new Set(M.map(N=>N.name))],color:Y(`${A} ${h.scale.replace(/_/g," ")}`)}})),f=i.scenes.findIndex(h=>h.handle.id===n.handle.id),x=f>=0?f+1:i.scenes.length,k=i.tracks.filter(h=>h instanceof F),y=k[0],w=k[1];if(!y){console.log("[composition-aide] No MIDI track found for the chord clips.");return}p.some(h=>h.bassNotes)&&!w&&console.log("[composition-aide] Only one MIDI track \u2014 bass clips skipped (add a second MIDI track).");for(let h=0;h<p.length;h++){let b=p[h],C=x+h,v=await i.createScene(C),I=X(`${q[b.section.key]??"C"} ${b.section.scale.replace(/_/g," ")}`);v.name=`${b.section.name} \u2014 ${I} \xB7 ${b.section.bars} bars`;let M=y.clipSlots[C];if(M){let T=await M.createMidiClip(b.totalBeats);T.notes=b.chordNotes,T.name=`${b.section.name} \xB7 ${b.chordNames.join(" ")}`,b.color!==null&&(T.color=b.color)}if(b.bassNotes&&w){let T=w.clipSlots[C];if(T){let P=await T.createMidiClip(b.totalBeats);P.notes=b.bassNotes,P.name=`${b.section.name} \xB7 Bass (${b.section.bass.replace(/_/g," ")})`,b.color!==null&&(P.color=b.color)}}}let _=p.reduce((h,b)=>h+b.section.bars,0);console.log(`[composition-aide] Song form: ${p.length} scenes, ${_} bars (${p.map(h=>h.section.name).join(" \u2192 ")})`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("Scene","Compose Song Form\u2026","aide.songForm"),t.commands.registerCommand("aide.theoryMachine",d=>{(async o=>{let n=t.getObjectFromHandle(o,J),i=t.application.song,s=i?.rootNote??0,u=de[i?.scaleName??"Major"]??"major",a=Pe.replace("</head>",`<script>window._INIT={key:${s},scale:${JSON.stringify(u)}};</script></head>`),r=await t.ui.showModalDialog(`data:text/html,${encodeURIComponent(a)}`,1e3,700),l;try{l=JSON.parse(r)}catch{return}if(!l||l.action!=="writeClip")return;let{chords:p,beatsPerChord:f,totalBeats:x}=l,k=se(p.map(E=>E.notes),f,l.rhythm),y=p.map(E=>E.name).join(" \u2013 "),w=n.clip;if(w instanceof B)w.notes=k,w.name=y;else{let E=await n.createMidiClip(x);E.notes=k,E.name=y}console.log(`[composition-aide] Theory Machine: wrote ${p.length} chords (${f} beats each) \u2014 "${y}"`)})(d).catch(o=>console.error(o))}),t.ui.registerContextMenuAction("ClipSlot","Modal Explorer\u2026","aide.theoryMachine")}0&&(module.exports={activate});
